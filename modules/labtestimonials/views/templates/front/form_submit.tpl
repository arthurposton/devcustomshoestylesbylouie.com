  ...
  {if $errors}
    <div class="alert alert-danger">
      {foreach from=$errors item=error}
        <p>{l s={$error} mod='labtestimonials'}</p></br>
      {/foreach}
    </div>
  {/if}
  {if $success}<div class="alert alert-success">{l s={$success} mod='labtestimonials'}</div>{/if}
   <div class="col-xs-6 col-md-12 form-submit">
    <form name="fr_testimonial" id="fr_testimonial" action="" method="post" enctype="multipart/form-data">
      ...
      <div class="form-group">
        ...
        <div class="form-group">
            <div class="col-sm-5">
         <button id="submitNewTestimonial"  class="button btn btn-default button-medium" name="submitNewTestimonial" type="submit" value="{l s='Send Your Testimonial' mod='labtestimonials'}"/><span> send </span></button>
        </div>
